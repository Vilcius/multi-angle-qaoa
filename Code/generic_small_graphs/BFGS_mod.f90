...
& dg(NMAX),g(NMAX),hdg(NMAX),hessin(NMAX,NMAX), pnew(NMAX),xi(NMAX),g2(NMAX)
logical :: grad_zero
...
do its=1,ITMAX  
	...
	do i=1,n 
		...
		xi(i)=pnew(i)-p(i) 
		!print *, 'xi(i),pnew(i),p(i) ',xi(i),pnew(i),p(i) 
		p(i)=pnew(i)
	enddo
	...
	do i=1,n
		!print*, 'xi(i),p(i),max(abs(p(i)),1.d0)',xi(i),p(i),max(abs(p(i)),1.d0)
		temp=dabs(xi(i))/max(dabs(p(i)),1.d0)
		if(temp.gt.test)test=temp 
		!if(temp.gt.test)print *, 'test:',test
	enddo
	if(test.lt.TOLX) then
		!print*, 'test.lt.tolx',test
		return 
	endif
